...
{
	// pour la coh�rence interne du dossier, il est fondamental que la collection
	// de ses �l�ments soit inaccessible aux clients.
	...
	//ajoute l'lment e dans le dossier
	{
		...
		rslt = elements.add(e) ; //ajout de l'lment e  la collection
		update_contenu();
		return rslt;
	}
	...
	//supprime et retourne vrai si la suppression de l'element e s'est bien passe, faux sinon (e non prsent ou erreur)
	{
		...
		{
			if (s.name==e.name)
			rslt = elements.remove(s) ; // suppression de l'lment e dans la collection
			update_contenu();
			return rslt;
		}
	...
	public void accept (Visitor v){
		...
		v.visitDirectoryAfter(this);
	}

	public Collection<ElementStockage> getElements() {
		//ne devrait pas exister, mise pour l'examen
		return elements;
	}
}
